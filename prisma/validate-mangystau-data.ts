...
const settlements = readJson<Settlement[]>('prisma/data/mangystau-settlements.json');
const distances = readJson<Distance[]>('prisma/data/mangystau-distances.json');
const cargoScenarios = readJson<CargoScenario[]>('prisma/data/mangystau-cargo-scenarios.json');
const analytics = readJson<AnalyticsFixtures>('prisma/data/mangystau-analytics-fixtures.json');
...
const duplicateCount = (values: string[]) =>
  [...new Set(values.filter((value, index) => values.indexOf(value) !== index))].length;
...
const invalidSettlements = settlements.filter((settlement) =>
  ...
  settlement.latitude < 40 || settlement.latitude > 50 ||
  settlement.longitude < 45 || settlement.longitude > 60,
);
const invalidDistances = distances.filter((distance) =>
  !settlementIds.has(distance.originId) ||
  !settlementIds.has(distance.destinationId) ||
  distance.distanceKm <= 0 ||
  distance.estimatedDurationMinutes <= 0,
);
const invalidCargoScenarios = cargoScenarios.filter((scenario) =>
...
const duplicateCoordinates = duplicateCount(
  settlements.map((settlement) => `${settlement.latitude},${settlement.longitude}`),
);
